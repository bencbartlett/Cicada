...
**Current status: v0.1 is UNDERWAY (work order: docs/17; order decided with Ben 2026-08-19 — DECISIONS.md row of that date): (0) fold `corpus/` into `examples/wall/` + `tools/` — DONE 2026-08-20; (1) undo/redo with the atomic `batch`/`apply_text` path + `#off` — DONE 2026-08-20; (2) git panel slice 1 — DONE 2026-08-20 (`wt/git-panel`: status markers, commit, revert; the chip, Git tab, badges, `Ctrl+S`); OCCT probe — DONE 2026-08-20, GREEN on Windows (docs/probes/occt-2026-08.md), then (3) OCCT-backed Solid as the main geometry track — WP-A (seam, build, CI), WP-B (the `Solid` kind end to end, the sharing model, display) and WP-C + WP-D (the OCCT-backed node set under the bare names, the mesh tier as `mesh_*`, `occt` ON by default, `examples/07-simple-cad.cic`) DONE 2026-08-20, WP-B's second review closed 2026-08-21 (tiered display off the session lock, the stale-pcurve transform fix, typed kernel refusals — docs/17 §Item 3) — follow-ups open (docs/17 §Follow-ups: the costed cancellable display edge, the doc-12 kernel worker, 02-solids' export node in the slider cone); (3b) scheduler foundations + compute-on-release — DONE 2026-08-20 (`wt/sched`: engine half and the sliders' `preview_policy` rendering); (4) time transport — DONE 2026-08-20 (`wt/transport`: engine + the web's play bar — the scrubber, `Space`, hidden transport-driven ports, the e2e); (5) scrub caching — **S1 (engine) DONE 2026-08-24** (`wt/scrub`: `slider(…, scrub=True)` at version 2; `crates/cicada-server/src/scrub.rs` — eligibility as a pure function, `SCRUB_MAX_POSITIONS` = 32, the generic `WarmQueue`; the `cicada-scrub` worker through the idle class — nearest-first, one position at a time, dry-run skips, the 256 MiB cap via `DiskStore::stored_bytes`, dropped on a text change, blocked by a live drag / playback, parked after a pre-emption; `ParamView.scrub`, `scrub_progress`, `set_scrub`, `/debug/state.scrub`; `slider_loop.mjs --snap --expect warm` is the DoD sweep), **S2 (web) DONE 2026-08-24** (`wt/scrub`: the one buffer bar under both slider widgets — `web/src/canvas/ScrubBar.tsx`, a segment per position, warm ones filled, the thumb's notch ringed, a pulse while warming — the toggle in the inspector's actions, the params row and the node menu greyed with the SERVER's `ineligible` reason — `web/src/panels/ScrubToggle.tsx`, `web/src/state/scrub.ts` — `scrub_progress` kept as a store overlay beside the graph; the `scrub` port row stays on the canvas; `web/e2e/scrub.spec.ts`: 02-solids warm / toggled / undone, and a Python-burn pipeline proving the compute-on-release tie-in — warm positions preview live with zero computed nodes, a cold un-scrubbed slider shows the pending chip on writer and observer); (6) WASM host last; track C (catalog: one-node-per-file restructure + node-format conformance — C0 DONE 2026-08-20; C1, the first docs/08 S+1 tranche — DONE 2026-08-20; `cicada mcp` — DONE 2026-08-20; Grasshopper names in search-to-place — DONE 2026-08-20; C2a, the 12 Point · Vector · Plane nodes + `examples/09-vectors.cic` — DONE 2026-08-24; C2b, the Transform rows `rotate_axis` / `scale_nu` / `polar_array` / `rectangular_array` / `compose_xform` / `transform` + the producer `construct_xform` over cicada-geom's new `Affine` (a non-similarity carries only what it carries exactly and refuses the rest, typed), `center_box`, `mesh_plane`, and the dropdown param `choice` (docs/10's name for GH's Value List; `ParamView` kind `choice` + `options`, a `<select>` on both param surfaces, `web/e2e/choice.spec.ts`) + the pegboard in `examples/06-lists.cic` — DONE 2026-08-24; next C3+) runs in parallel throughout. **Wave 4's usage tracks — DONE 2026-08-24** (docs/17 §Usage findings 2026-08-24; DECISIONS.md row of that date): `cicada app` (serve + the app window; `tools/launch/Cicada.cmd` / `Cicada.command` build-if-stale-and-run; `tools/launch/bundle.py` → a redistributable folder with the run-time libraries beside the binary — `tools/fetch_occt.py --bundle`); `cicada serve` without a path serves the HOME directory as the root and opens nothing — `GET /api/files` (root-confined listing), File → Open / Recent / Close + the landing picker, a dead pipeline refused inside the handshake, `hello.role = observer` for the pop-out viewport (`?view=viewport`); the canvas: dimmer grid tokens, the viewport gimbal (`--axis-x/y/z`), value summaries from the `near` tier, the trace router (`web/src/canvas/trace.ts`), typed literals on unconnected inputs (canvas chips + inspector; `set_param` adds a missing kwarg), `place_node.params` + `set_collapsed` (collapsed sliders, the `A<B<C` shortcut). **User-test round 2 — 2026-08-25** (docs/17 §Usage findings 2026-08-25: findings U15–U34): the fast lane landed the same day — two LOD states (`lodTier` = `far` | `near` | `closest`, no `mid`; the far tier keeps the handles where the face has them), the eye / eye-off icon, the wire glow on hover and on a selected node's wires, GH's single / double / thick-dashed wire convention, the timing badge (`(1.2ms)` grey for a memo hit, three-significant-figure hovers, `ns`), four-significant-figure face values (`web/src/canvas/valueText.ts`), ribbon placements at the view's centre (`store.canvasCenter`); U30/U31 measured (§Measurement U30 / U31: 1,000 fine-tier B-rep spheres = 8,000 triangles each, a 160 MB frame, 2.3–2.5 s of tessellation per redraw, re-paid on every undo/redo because two value sets thrash the 256 MiB solid cache; refs, memo and canvas exonerated; the status bar's time is the solve alone); **wave 5 CONFIRMED 2026-08-25 and UNDERWAY** (docs/17 §Wave 5 — contracts frozen; DECISIONS.md rows of the date): Round 1 — `wt/display` — **DONE 2026-09-19, merged** (D1, the display edge bounded and visible: `display::DISPLAY_TRIANGLE_BUDGET` = 1 M triangles per output per generation — above it the output is drawn at the preview tier and `stats.budget` says so, a pure function of the value set memoized per (value hash, tier) in `VerdictMemo`; the solid display cache 1 GiB by default and resizable live — `cicada serve --solid-cache-mib`, the writer-only `set_display_cache` intent, a per-user choice in the settings menu the lease holder re-applies — with `SolidCache::watch` touching the on-screen working set so a fitting picture is never evicted; the `caches` view in every snapshot and after every pass (the display cache's counters and the `over_budget` / `thrash` flags, the memo store's bytes via `DiskStore::value_bytes`), ONE warning notice when a flag rises or a structural redraw meets a standing one, never per tick; the caches chip `cache 26M / 1G · 269 meshes` (click → the profiler's caches section); the chip `Solving gen N` → `gen N · painting…` → `gen N · solve 17 ms · display 2.4 s`, `cancelled gen N` when Esc cut the pass, the counts in the hover; `display_begin {generation, outputs}` on the control lane before the tessellation and `display_end {…, cancelled, cut_by: edit | esc}` on the display lane behind the last frame, the viewport's `painting N outputs…` spinner and `painted … in T`; latest-wins for the display pass is the SOLVE's — a pending structural job or Esc cuts it between outputs and says so, a preview or transport tick never does (the first rule starved every drag and playback), the encode draws only what the warm-up pinned and never tessellates under the lock, a preview toggle repaints through an announced display-only generation off the lock; `web/e2e/display.spec.ts`. P1, the profiler: the `profile {generation?}` read → `profile_view` — the last complete generation's phases (queued, solve, tessellate, encode, bytes), per-node costs (computed: measured; cached: the memo entry's last cost, marked; `idle` outside a preview's cone), the display rows recorded with the pass (tier, triangles, bytes, cache hits / misses shared by every output of one value), the caches, `cancelled` / `cut_by` for a cut pass — also `/debug/state.profile` and `timings[].solve_ms`; the client adds its own phases (decode, upload, first paint, the socket residual — sealed once the pass lands, a restream adds nothing) and asks once per landed pass with one read outstanding, a refusal shown as the placeholder; the Profile inspector tab — an SVG ring, the nodes and display tables with totals, sortable, a filter, the caches section with the budget control, Esc closes only it — and the `profile` button beside the chip; `web/e2e/profile.spec.ts`), `wt/face` — **DONE 2026-09-19, merged** (N1: the collapse chevron on the slider's face in both states — absolutely positioned on the expanded face, before the output handle on the collapsed row; greyed with the mirror's reason, the server still decides — the collapsed value as the chip editor on double-click (one `set_param` through `paramValueText`), the name-first collapsed row as a CSS grid whose track floor is an exact 40 % measured inside the body, the buffer bar on the track's column, and `inspect` answering `inputs` beside `outputs` — a wired input shows its source output's summary on the face and in full in the inspector; the fix round: a wire out of a multi-target line is spelled as its node + the port the target unpacks (`WireEnd` — the canvas, `inspect_wire`, `inputs`, `connect` and `probe_wire` agree), the inspect summary reads the display cache and never tessellates (a hidden solid says `not displayed`), `Core::summaries` memoizes each kept generation's summaries off the session lock (`/debug/state.summaries`), the canvas inspects red and blocked nodes too, an older engine's `node_values` without `inputs` reads as none; `nodeFace.test.tsx`, `inputValues.test.tsx`, `web/e2e/slider.spec.ts`, `visuals.spec.ts`), `wt/menu` — **DONE 2026-09-19, merged at f35942d** (C2c: the required `#[node(sub = …)]` attribute on every node, `spec::SUBGROUPS` — one table in cicada-core that docs/08 mirrors and the conformance test enforces — catalog format 3 with `sub` per node and the `subgroups` table, `CATALOG.md` grouped by sub-group, `cicada mcp` carrying `sub`, a blank / padded / missing `sub` each a trybuild witness; M1: the ribbon replaced by a **menu bar** of sub-group dropdown panels — `web/src/panels/MenuBar.tsx`, closed by an outside click, Esc, a re-click or a placement, which lands at the view's centre; a catalog of another format is refused and said in the bar and the search box; `ribbonCollapsed` is gone; `web/e2e/menu.spec.ts`); Round 2 — `wt/viewport` — **DONE 2026-09-20, merged** (V1: `settings.viewportMode` = split · floating · window, a three-way control in the viewport toolbar and in settings — *split* the panes as before (its presets and the swap greyed outside it); *floating* the canvas across the whole work area with the viewport in a panel over it, dragged by its strip and resized from its corner, its rect a per-user setting clamped into the area, minimum 240 × 160, the same scene; *window* the browser's Document Picture-in-Picture window with the viewport ELEMENT moved into it — the WebGL context survives the move, the toolbar works through a portal into the host, the PiP's title follows the pipeline — a placeholder in the main window brings it back, the PiP closing on its own returns to split, leaving by the control lands on the chosen mode, a stored `window` loads as split (the PiP dies with its page and `requestWindow` needs a gesture); without the API — an insecure origin is named as such — the wave-4 observer pop-out opens instead with one notice per window, and that pop-out now lives under settings › viewport › second monitor; the pane swap reorders keyed children and remounts nothing; `web/src/viewport/{modes.ts, windowMode.ts, ViewportFrame.tsx}` with their vitests, `web/e2e/viewport_modes.spec.ts` — the real PiP on 127.0.0.1 in headless Chromium, skipped without the API, the stubbed fallback a test of its own), `wt/about` — **DONE 2026-09-20, merged** (R1, releases and About: the workspace is `0.1.0-alpha.1`; `crates/cicada-cli/build.rs` + `src/stamp.rs` stamp the build — the commit (short-12, `-dirty` re-taken on every change to `stamp::BUILD_INPUTS` — `crates/`, `web/`, the manifests — a deleted input watched through its directory; `CICADA_GIT_SHA` for CI checkouts; `unknown` said on stderr) and the date (HEAD's commit date; `SOURCE_DATE_EPOCH` wins; a clock that gives none fails the build) — into `cicada --version`, `hello.version {semver, commit, built}` + `hello.threads` (the scheduler's resolved count) and `GET /api/version`; **About** in the settings menu is a store modal (`aboutDialog`; `store::modalOpen` — one modal at a time, the keyboard map closes it on Esc first and every other hotkey is inert behind it, focus returns to the gear, which hands a plain Esc on to the map); `CHANGELOG.md` with `tools/changelog.py check --tag` refusing its TAG-TODO; `.github/workflows/release.yml` on `v*` tags — builds and bundles Windows and Apple-silicon macOS with `bundle.py --check` (the binary's `--version` held to the stamp and the README), ships the Linux bare binary, has a `workflow_dispatch` dry run that publishes nothing, grants the write token to the publishing job alone, and REFUSES to publish without `LICENSE` and `THIRD_PARTY_NOTICES.md` (both carried in the bundles) — **the alpha tag is held until Ben writes those two files** (DECISIONS rows 2026-08-11 and 2026-08-20); `tools/release_stamp.sh`; `crates/cicada-cli/tests/version.rs`, `web/e2e/about.spec.ts`, `AboutDialog.test.tsx`, `CommitDialog.test.tsx`, `tools/test_changelog.py`. T1, tooltips at 250 ms: `web/src/tooltip.ts` + `TooltipLayer.tsx` — one capture-phase listener set on the document, `TOOLTIP_DELAY_MS` = 250 held by a test; every `title=` site (150 at the time) AND the SVG `<title>` hovers of the wires and the profiler arcs (those below the pointer) show in a themed box below the element, above when there is no room, clamped to the viewport; the `title` is parked in `data-title` (the attribute left empty, so the browser's own tooltip never doubles it) and restored on leave, a title the app rewrites under the pointer is adopted through a MutationObserver; no tooltip while a button is held (drags), the element under the release is entered; disabled controls keep the native tooltip; docs/16 §Theme and visual language 'Tooltips'; `tooltip.test.ts` (fake timers), the smoke spec's in-page timing assertion); the 02-solids export chain is `#off` by default. Next: the follow-ups (docs/17 §Follow-ups; item 5's: re-centring a scrubbed slider's queue on its own text change instead of dropping it). docs/17 §Scope carries the live status table — update it when an item moves.**
...
call a typed `KernelUnavailable`). **Since 2026-08-24 the same
...
- **A worktree may hold another agent's uncommitted work.** Never clean
  ...
  they had to be replayed from that agent's transcript. The same rule
  for processes: **never stop a `cicada` by image name** (`taskkill /IM
  cicada.exe`, `pkill cicada`) — another agent's server, or the
  Playwright suite's engine on 8471, is a `cicada` too (2026-08-25: a
  measurement's cleanup took the suite's engine down mid-run). Kill the
  PID that owns YOUR port (`netstat -ano | findstr :<port>` → `taskkill
  /F /PID <pid>`; `lsof -i :<port>` elsewhere), and serve every
  experiment on a port of its own.
