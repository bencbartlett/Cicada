...
| MCP server for agents | `cargo run -q -p cicada-cli -- mcp [--project <dir-or-pipeline.cic>]` — stdio JSON-RPC (stdout is the protocol; notes on stderr); tools `catalog_search`, `node_doc`, `list_categories`, `check`; `--project` adds that project's `scripts/*.py` to the catalog and anchors relative `check` paths. Register via `.mcp.json.example` (copy to `.mcp.json`; Claude Code picks it up from the repo root) or point any MCP client at the built binary `$CARGO_TARGET_DIR/debug/cicada mcp` |
