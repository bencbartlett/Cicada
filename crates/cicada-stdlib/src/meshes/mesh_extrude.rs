//! The `mesh_extrude` node — the spike's mesh-backed extrusion, continuing
//! the mesh tier under its `mesh_` name since the OCCT-backed `extrude`
...
/// Inputs for [`mesh_extrude`].
#[derive(Ports, Clone, Debug)]
pub struct MeshExtrudeIn {
...
/// Mesh Extrude — extrude a closed planar profile into a watertight prism
/// mesh (the mesh tier's extrusion: curved profiles tessellate to
/// `segments` edges — the wall's frusta and cutters; GH reaches it with
/// Extrude then Mesh Brep; the B-rep `extrude` is the default working
/// mode).
///
/// # Returns
///
...
/// prism = mesh_extrude(profile=ring, direction=up)
/// ```
#[node(
    category = "Mesh & field",
    tier = "S",
...
#[must_use]
pub fn mesh_extrude(config: &ProjectConfig, input: MeshExtrudeIn) -> Watertight<Mesh> {
...
mod tests {
    ...
    #[test]
    fn mesh_extrude_is_watertight_with_expected_volume() {
        let prism = mesh_extrude(
            &config(),
            MeshExtrudeIn {
    ...
    fn extrude_circle_one_past_the_ceiling_is_red() {
        ...
        assert_eq!(
            *panic.downcast_ref::<String>().unwrap(),
            "mesh_extrude: segments is 4194305 — above the 4194304 (2^22) slot ceiling of one node \
             output"
        );
    }
    ...
    #[test]
    #[should_panic(
        expected = "mesh_extrude: segments is 100000000000 — above the 4194304 (2^22) slot ceiling of \
                    one node output"
    )]
    ...
    fn mesh_extrude_in_plane_direction_is_red() {
        let _ = mesh_extrude(
            &config(),
            MeshExtrudeIn {
                profile: unit_square_profile(),
    ...
    #[test]
    fn mesh_extrude_determinism_golden_hash() {
