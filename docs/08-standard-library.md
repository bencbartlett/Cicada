...
| Value List | `choice(value: Text, options: [Text]) → Text` | 1 | **shipped 2026-08-24 (C2b)** as `choice` — the dialect's name for the dropdown param since the first draft of docs/10 §3 and the DECISIONS.md dialect-grammar row (`slider` / `toggle` / `choice` / bare literals are the constructor bindings the canvas renders as widgets); the C2b contract's `value_list` was the GH name snake-cased, and the ledger's name won. The option IS the value (a `Text` a node downstream reads; GH's label-per-value pairing is not modelled — a `[Text]` of labels and a second list of values is the user's to keep in step); red when `value` is not one of `options` (a renamed or removed option leaves a drifted literal — loud, never a silent fall-back to the first option), an empty `options` included. The widget (docs/16 §Canvas conventions): a `<select>` over the literal `options` on the canvas and in the params panel, one `set_param` spelling the option as a Text literal; the `ParamView` kind `choice` carries `options` (docs/13). `gh = "Value List"` |
...
| Box / Center Box | `box(plane: Plane = xy_plane, x, y, z: Domain) → Solid` | S | **shipped 2026-08-20 (WP-C), OCCT-backed** (`BRepPrimAPI_MakeBox` in the plane's frame; decreasing domains normalize; the world-frame box is byte-identical to the seam's `box_at`). The spike's mesh-backed box continues as `mesh_box` (§8). **Center Box shipped 2026-08-24 (C2b)**: `center_box(plane: Plane = xy_plane, x: Number = 1.0, y: Number = 1.0, z: Number = 1.0) → Solid` — `box` over the mirrored domains `-x..x`, `-y..y`, `-z..z` in the plane's frame (byte-identical to that `box`; the half-extents are Grasshopper's, `x = 1` spans two units and the defaults make its 2 × 2 × 2 cube); a non-positive half-extent is red by name before the kernel sees a domain |
...
| Rotate | `(geometry: T, angle: Number, plane: Plane) → T` | S | **Rotate Axis shipped 2026-08-24 (C2b)**: `rotate_axis(geometry: T, angle: Number, axis: Curve) → T` — the rotation about the line through a `line`'s start along its direction (any length), the same matrix `rotate` applies about a frame's z (`Similarity::rotation_about_axis`; a property test holds the two nodes to each other); `axis` is a `line` as `revolve`'s is — any other curve kind is red, so is a zero-length one |
| Scale | `(geometry: T, center: Point, factor: Number) → T` | S | **Scale NU shipped 2026-08-24 (C2b)**: `scale_nu(geometry: T, plane: Plane = xy_plane, x: Number = 1.0, y: Number = 1.0, z: Number = 1.0) → T` — a factor per axis of the plane's frame about its origin (`cicada_geom::transform::Affine::scale_in_frame`). Equal factors are the uniform `scale` exactly (every kind, a Solid through the kernel); unequal factors carry only the kinds a stretch carries EXACTLY — points, vectors, lines, polylines, meshes (windings swap on a negative factor); a plane while its axes stay perpendicular; a circle while its own plane is scaled evenly (an XY circle under a z-only stretch stays a circle); a rectangle aligned with the frame, its domains scaled — and refuse, typed with the numbers (`GeomError::AffineRefused`), a circle that would be an ellipse (no such kind), a skewed frame, and any Solid (the kernel's general transform `BRepBuilderAPI_GTransform` is not wired — docs/17 follow-up; `gp_Trsf` would silently re-orthogonalize a shear, so a non-similarity never reaches it). A factor within tolerance of zero is red |
...
| Polar / Rectangular Array | `polar_array(geometry: T, plane: Plane = xy_plane, count: Integer, angle: Number = 2π) → [T]` / `rectangular_array(geometry: T, plane: Plane = xy_plane, cell: Vector, x_count: Integer, y_count: Integer, z_count: Integer = 1) → [T]` | 1 | **shipped 2026-08-24 (C2b)**. Polar: `count` copies turned about the plane's normal through its origin, the original first; the fence posts are the node's contract — a full turn (`\|angle\|` within angular tolerance of 2π, the default) divides the circle, `k × angle / count`, the last copy NOT on the first; any shorter sweep is FILLED, `k × angle / (count − 1)`, the last copy on the sweep's end — Rhino's `ArrayPolar` rule (its "angle to fill"), NOT Grasshopper's: the GH component divides every sweep by `count` (McNeel RH-81087 — ten copies over 90° end at 81°), so a `.gh` import translates the angle; the switch is at the angular tolerance, so a typed approximation of 2π (`6.28`) fills, its last copy a hair before the first — omit `angle` for the full turn; negative turns the other way; a zero sweep stacks the copies; beyond a full turn is red; one copy is the original. Rectangular: `x_count × y_count × z_count` copies stepped along the plane's axes by the `cell` vector's components (GH's Cell box reduced to its three extents; a negative step runs the other way), ordered x fastest, then y, then z; the PRODUCT is charged against the shared ceilings, every copy costed with its payload like `linear_array`'s. Both take a Solid through the kernel |
