...
| Polar / Rectangular Array | `polar_array(geometry: T, plane: Plane = xy_plane, count: Integer, angle: Number = 2π) → [T]` / `rectangular_array(geometry: T, plane: Plane = xy_plane, cell: Vector, x_count: Integer, y_count: Integer, z_count: Integer = 1) → [T]` | 1 | **shipped 2026-08-24 (C2b)**. Polar: `count` copies turned about the plane's normal through its origin, the original first; the fence posts are the node's contract — a full turn (`\|angle\|` within angular tolerance of 2π, the default) divides the circle, `k × angle / count`, the last copy NOT on the first; any shorter sweep is FILLED, `k × angle / (count − 1)`, the last copy on the sweep's end, as Rhino's and Grasshopper's polar arrays fill an angle; negative turns the other way; beyond a full turn is red; one copy is the original. Rectangular: `x_count × y_count × z_count` copies stepped along the plane's axes by the `cell` vector's components (GH's Cell box reduced to its three extents; a negative step runs the other way), ordered x fastest, then y, then z; the PRODUCT is charged against the shared ceilings, every copy costed with its payload like `linear_array`'s. Both take a Solid through the kernel |
