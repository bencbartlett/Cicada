...
| Weave / Merge / Insert Items / Split List | `weave(pattern: [Integer], a: [E], b: [E]) → [E]` / `insert_items(list: [E], items: [E], indices: [Integer]) → [E]` / `split_list(list: [E], index: Integer) → (a: [E], b: [E])` | 1 | slot-preserving throughout; all shipped (C1); Merge = `concat`. `weave` is the two-stream form (`0`/`1` turns, the pattern repeats until both streams are used up; a turn for an exhausted stream while the other has slots is red — GH pads nulls there). `insert_items` applies its insertions in order (each index addresses the list as the previous insertions left it, as GH does); no `wrap` — an index past the current length is red |
