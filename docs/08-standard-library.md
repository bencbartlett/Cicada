...
| map / zip / cross / nest / squeeze / transpose / group_by / compact | list & nesting combinators | S | wire-level lifts with node forms; strict `zip` (mismatch = error; the opt-in adapters are the nodes `pad_last(list, count)` / `repeat(pattern, count)` / `truncate(list, count)` — shipped C1); shipped C1 as nodes: `nest` (Graft), `transpose` (Flip Matrix; rectangular only, ragged is red), `group_by(keys: [Number], values: [E]) → (groups: [[E]], keys: [Number])` (first-occurrence order), `compact(list: [E?]) → (values: [E], map: IndexMap)` (an `E?` port keeps the wired `?` on the port, so `values` types present — the checker's `compact` advice is satisfiable; C1 review fix); `cross` needs a second type variable and `squeeze`'s depth is data-dependent — both pending; doc 09 |
