...
| 3 | OCCT-backed Solid — the `Solid` kind, primitives/extrude/loft/revolve/sweep, booleans, `tessellate`, STEP; `mesh_*` renames in the same commit | main geometry track from week 3 | weeks | **WP-A done** 2026-08-20 (fork `bencbartlett/opencascade-rs@38ed25e`, `occt` feature + seam in `cicada-geom`, `tools/fetch_occt.py`, CI jobs `occt (ubuntu)` per PR and `occt (<os>)` nightly — the non-Windows jobs await their first run); WP-B next |
...
- **WP-A seam + build + CI** — **done 2026-08-20** (docs/03 §The OCCT
  seam as built): feature-gated `occt` in `cicada-geom` over Ben's fork
  `github.com/bencbartlett/opencascade-rs` branch `cicada` @
  `38ed25e1dd61b5798f257269397571f7b6f8ba48` (upstream `d114250` + the
  MSVC handle aliases, the honest `BinTools`/`BRepTools` writers, in-memory
  serialization, the `Standard_Failure` → `Err` boundary, the `cicada`
  glue); `tools/fetch_occt.py` + `tools/fetch_occt_manifest.json`
  (conda-forge 7.8.1 build 103 for win-64/linux-64/osx-64/osx-arm64 + the
  run-time closure, sha256-pinned, user cache dir, static closure check,
  `--print-env`); deny rows for `opencascade-sys` + the fork source; CI
  jobs `occt (ubuntu)` (ci.yml, per PR) and `occt (<os>)` (nightly.yml,
  three OSes). Seam surface: `occt::Solid` with `box_at`,
  `extrude_polygon`, `difference`, `tessellate → Watertight<Mesh>`,
  `canonical_bytes`/`from_canonical_bytes`; 14 tests incl. golden blake3
  hashes of the canonical bytes. Measured on Windows; the Linux/macOS
  jobs are unverified until they run. Left for WP-B: the `Solid` value
  kind over `occt::Solid` (Send, not Sync — decide the handle-cache
  sharing model), per-OS goldens until the nightly shows agreement;
