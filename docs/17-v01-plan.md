...
**Landed 2026-09-19..20** — every package merged per track, each
verified in its worktree with main merged in (the whole Rust and web
battery, the wall carve hash unchanged, the full Playwright suite): `wt/menu`
(C2c + M1) at f35942d, `wt/face` (N1) at 317a72c, `wt/display` (D1 + P1)
at 9d9170c, `wt/viewport` (V1) at 8e92818, `wt/about` (R1 + T1) at
394495d; CI green on each fast-forward. The per-package closure
paragraphs are under each contract below. What the merges taught,
recorded where it bites: the full suite at a merge found two suite-order
couplings on the one served copy of 02-solids (the profiler and smoke
specs now write their own preconditions), and the first CI run after the
display merge found three tests that held only on the Windows dev
machine — a session test racing the structural debounce, and two
Playwright assertions that were facts about the runner's font (the brief
now names font, timing and retry assumptions as a review lens; AGENTS.md
dev notes). The usage limits that cut the workflow runs short three
times cost work only where an agent had not committed: the standing rule
for fix agents is one commit per finding, and a cut-off agent's dirty
tree is read and continued by the next, never discarded. The alpha tag
is **held**: the release workflow refuses to publish without `LICENSE`
and `THIRD_PARTY_NOTICES.md`, which are Ben's to write (DECISIONS rows
2026-08-11 and 2026-08-20; R1's fix-round record lists what the notices
must carry); the CHANGELOG's 0.1.0-alpha.1 section is tag-ready
(`tools/changelog.py check --tag v0.1.0-alpha.1`).
...
- **R1 — releases and About.** The workspace version becomes
  ...
  *Built 2026-09-19 (`wt/about`).* The workspace version is
  ...
  `CHANGELOG.md` + `tools/changelog.py`
