...
- `duplicate(item: E, count: Integer) → [E]` — Duplicate · GH: Duplicate Data — a list of `count` copies of one element (the idiomatic singleton list is `count=1`: geometry lists come from nodes, list literals hold scalars). An absent element duplicates into absent slots (`E` carries the `?`). Red when: `count` is negative (`0` is the empty list).
...
- `linear_array(geometry: T, direction: Vector, count: Integer) → [T]` — Linear Array · GH: Linear Array — `count` copies stepped along a direction, the first at the original position. Red when: `count < 1`.
