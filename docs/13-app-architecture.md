...
| `GET /api/git/status` | `?pipeline=` → `{state, pipeline: {path, tracked, dirty, nodes: [{name, change, from?}], removed: [{name, line_in_head}]}, scope: [{path, status}], text_hash}` (doc 10 §Git integration, slice 1: working tree vs HEAD). `state` is tagged `kind`: `repo {root, prefix, branch, head_short, upstream: {name, ahead, behind}?, unborn}` \| `not_a_repo` \| `git_not_found` \| `locked` (`index.lock` held — status still answers, writes wait). `change` ∈ `added` / `modified` / `removed` / `renamed` (`from` = the HEAD name); markers are computed FROM `git diff -U0 HEAD -- <path>` (hunks → binding lines, one binding per line), so they cannot disagree with it; the sidecar never marks a node; an untracked pipeline is every node `added`. `scope` = the dirty files of the commit scope — this pipeline's `.cic`, its sidecar, `scripts/*.py` beside it (the `apply_text` set), project-relative, `status` ∈ `modified` / `added` / `deleted` / `untracked` / `renamed`. `text_hash` = blake3 of the working file the markers were computed against (clients dedupe on it). Reads only: every invocation carries `--no-optional-locks`, so a refresh never touches the project and never wakes the watcher (CI-tested under a live watcher) |
| `POST /api/git/commit` | `{message, client?}` (writer-gated: `client` or `X-Cicada-Client` must be the lease holder — committing is a git action on the project, not a document edit, hence unlike `apply_text`) → `git add -- <scope>` then `git commit --cleanup=verbatim -F - -- <scope>` (the message verbatim on stdin; `-- <paths>` commits ONLY the scope, so whatever else the user staged in a shell stays staged; never `add -A`) → `{hash, short, summary, files}`. 422 `empty_message`, 409 `nothing_to_commit` / `not_a_repo` / `git_not_found`, 423 `locked`, 403 `lease`, 500 `git_failed` (with `command`, `code`, `stderr`) / `git_timeout` |
| `POST /api/git/revert` | `{paths?, client?}` (writer-gated as above) → `git checkout HEAD -- <paths>` for the dirty scope files that HAVE a HEAD version (`paths` narrows the set; 422 `path_not_allowed` outside the scope) → the session reloads through the external-change path (`reload_from_disk` → ONE barrier snapshot, `reason: "git revert"`, op log cleared; the watcher's later wake finds disk == memory and does nothing) → `{reverted, untracked, reloaded}`. Files without a HEAD version are never deleted: `untracked` lists the ones left alone; an untracked pipeline, or an explicit ask for one, is 409 `untracked`; 409 `nothing_to_revert`; 500 `reload_failed` when the files are back on disk but the session could not load them (previous state stays live). Every git error body is `{kind, message, …}` with `kind` the snake_case `GitErrorKind` enum in `protocol.rs`, mirrored by the client |
...
disabled node NAMED). Shipped with v0.1 item 2 (2026-08-20, `wt/git-panel`,
server half): the three `/api/git/*` routes above over the git binary
(`git.rs`; `GET /api/project` gained `scripts` and `git`). Still not in:
transport, the other git refs / graph-diff overlay / per-node history, `/api/blob`
