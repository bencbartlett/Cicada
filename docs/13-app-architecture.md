...
- **Single-writer lease** per pipeline: the first client takes the
  ...
  **The build on `hello`** (v0.1 wave 5 R1, additive — `PROTOCOL_VERSION`
  ...
  to the server through `ServeConfig::version` (`protocol::VersionInfo`;
  the library alone reports its semver with `unknown` for the rest) —
  and `threads`, the session's worker count as the scheduler resolved it
  (`--threads 0` = cores − 2 → the number). The app's About dialog
  (docs/16 §Settings) shows both; `GET /api/version` answers the same
  `version` object without a socket. `engine` (`cicada <semver>`) is
  unchanged.
...
restream resumes behind it. Control texts are small — the coalesced
...
**What stays ordered with the frames.** Three texts are display-plane by
meaning and ride the display lane, FIFO with the frames:
`display_reset`, the header of the restream it announces (the lane's
order is what puts it ahead of them), `screenshot_request`, which
renders what the frames before it painted — `GET /debug/screenshot`
after `/debug/state?wait=true` keeps meaning "the completed
generation" — and, since v0.1 wave 5 D1, `display_end`, which closes a
generation's pass behind its last frame (§The display edge; the
restream tests' `generation_text` allow-list in `http.rs` must never
admit it to the control lane).
...
(`display::summarize`). The two halves are split around the session lock
(fix round 2026-09-19, wave 5 N1 review CR-2 / C-4): the ports are
resolved to hashes UNDER it (cheap), the loads and summaries run OFF it —
so an inspect of a 1,000-element list holds up no delta and no generation
— and every distinct value is summarized ONCE per kept generation
whichever node asks (a memo by value hash, `Core::summaries`, emptied when
a newer generation is kept so the release's fine-tier facts replace a
drag's coarse ones; a producer's output and each consumer's input are one
entry; the canvas's per-generation sweep over the visible nodes therefore
costs one summary per distinct value, not one per port). A summary of a
solid the display never meshed says `tessellation: "not displayed"` and
is not memoized (docs/12 §Display cache). `GET /debug/state` →
`summaries` carries `generation`, `entries`, `computed`, `hits`. `inputs`
is additive (v0.1 wave 5 N1, finding
...
| `GET /api/catalog` | Node-spec JSON catalog (docs/08) — the stdlib plus the served project's script nodes. Format 3 since v0.1 wave 5 (C2c): every node carries `sub` (its sub-group within the category — the menu bar's column; `Script` for script nodes) and the catalog carries `subgroups`, one `{category, subgroups}` row per category in menu order (`cicada_core::spec::SUBGROUPS` — the client reads the column order here and keeps no copy). The client mirrors the number (`web/src/protocol/version.ts::CATALOG_FORMAT`, beside `PROTOCOL_VERSION`) and `fetchCatalog` REFUSES a body of any other format with an error naming both numbers — the state layer raises it as a notice, keeps the catalog it has and records the refusal in the store (`catalogError`, cleared by the next good read), so that a first connect with no catalog to keep shows the refusal in the menu bar's and the search box's empty states rather than `catalog loading…` — so a mixed-age engine and app (an older engine on the proxy port, a stale embedded SPA) is a loud refusal, never a menu silently grouped by category alone |
| `GET /api/version` | `{semver, commit, built}` — the build behind this server (v0.1 wave 5 R1): the object every `hello` carries as `version` (§Projects, pipelines, sessions), for a launcher, a script or an agent with no socket. Token-gated like every `/api` route; `/health` stays the bare `ok` the bundle's smoke (`tools/launch/bundle.py --check --smoke`) and Playwright's `webServer` wait on |
